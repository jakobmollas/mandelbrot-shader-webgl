...
function createWebGl2RenderingContext(canvas: HTMLCanvasElement): WebGL2RenderingContext {
...
function createVertexShader(gl: WebGL2RenderingContext): WebGLShader {
    const source =
        `#version 300 es
    ...
    const vertexShader = createShader(gl, gl.VERTEX_SHADER, source) as WebGLShader;
    return vertexShader;
}

function createFragmentShader(gl: WebGL2RenderingContext) {
    const source =
        `#version 300 es
    ...
    const fragmentShader = createShader(gl, gl.FRAGMENT_SHADER, source) as WebGLShader;
...
function createShader(gl: WebGL2RenderingContext, type: number, source: string): WebGLShader {
    ...
    const errorDetails = gl.getShaderInfoLog(shader) as string;
...
function createProgram(gl: WebGL2RenderingContext, shaders: WebGLShader[]): WebGLProgram {
    ...
    const errorDetails = gl.getProgramInfoLog(program) as string;
    throw new Error(errorDetails);
}

function resizeCanvasToDisplaySize(canvas: any): void {
